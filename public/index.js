function printer(day, date, iteration) {
    ...
    if (iteration < tasks[date].length) {
        document.getElementById(day).innerHTML = tasks[date][iteration] + "\xa0\xa0\xa0\xa0\xa0";
    }
}
...
function process() {
    today = new Date();
    var x = "", mm = today.getMonth() + 1, dd = today.getDate();
    if (mm <= 9) mm = '0' + mm;
    if (dd <= 9) dd = '0' + dd;
    ...
    if (tasks.hasOwnProperty(document.getElementById("date").value)) {
        tasks[document.getElementById("date").value].push(document.getElementById("assignment").value);
    }
    else {
        tasks[document.getElementById("date").value] = [document.getElementById("assignment").value];
    }

    nn = new Date((document.getElementById("date").value).replace(/-/g, '\/').replace(/T.+/, ''));
        ...
        if (dateify(nn) == x) {
            for (j = i; j <= document.getElementById("parts").value; j++) {
                if (tasks.hasOwnProperty(dateify(nn))) {
                    tasks[dateify(nn)].push(document.getElementById("assignment").value + "-prep");
                }
                else {
                    tasks[dateify(nn)] = [document.getElementById("assignment").value + "-prep"];
                }
            }
            break;
        }
...
function dateify(n) {
    var x = "", mm = n.getMonth() + 1, dd = n.getDate();
    if (mm <= 9) mm = '0' + mm;
    if (dd <= 9) dd = '0' + dd;
